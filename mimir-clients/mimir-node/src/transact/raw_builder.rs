...
impl<'a,S> RawTxBuilder<'a,S> {
    ...
    ///
    pub fn nonce<T: ?Sized>(mut self, nonce: &'a T) -> Self where T: AsRef<[u8]> { self.nonce = Some(nonce.as_ref()); self }
   
    /// set the value of the transaction gas price.
    ///
    pub fn gas_price<T: ?Sized>(mut self, price: &'a T) -> Self where T: AsRef<[u8]> { self.gas_price = Some(price.as_ref()); self }

    /// set the value of the transaction gas limit.
    ///
    pub fn gas_limit<T: ?Sized>(mut self, limit: &'a T) -> Self where T: AsRef<[u8]> { self.gas_limit = Some(limit.as_ref()); self }

    /// set the transaction destination address.
    ///
    pub fn to<T: ?Sized>(mut self, to: &'a T) -> Self where T: AsRef<[u8]> { self.to = Some(to.as_ref()); self }

    /// set the value (in wei) to be sent with this transaction.
    ///
    pub fn value<T: ?Sized>(mut self, value: &'a T) -> Self where T: AsRef<[u8]> { self.value = Some(value.as_ref()); self }

    /// set the transaction calldata.
    ///
    pub fn data<T: ?Sized>(mut self, data: &'a T) -> Self where T: AsRef<[u8]> { self.data = Some(data.as_ref()); self }
}


impl<'a,S> RawTxBuilder<'a,S> where S: Signer<Msg=[u8;32],Sig=Signature> {
    ...
    pub fn finish(self) -> Vec<u8> {
        ...
        let body = Body { nonce, gas_price, gas_limit, to, value, data };
